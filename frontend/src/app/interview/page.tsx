...
export default function InterviewPage() {
  ...
  // startTimeRef is no longer needed as startTime is managed by context
  ...
  useEffect(() => {
    if (!sessionId && questions.length === 0) {
      ...
      if (currentIdx === 0 && transcript.length === 1) {
        // Set startTime when the interview officially begins
        if (!startTime) {
  ...
  }, [sessionId, questions, currentIdx, transcript, startTime, setStartTime]); // Added startTime and setStartTime to dependencies
  ...
  const handleStop = async () => {
    if (sessionId) {
      // No longer using startTimeRef. Record duration via context startTime.
      const durationInSeconds = startTime ? Math.floor((Date.now() - startTime) / 1000) : 0;
      setSessionDuration(durationInSeconds);
      await interviewService.finalizeSession(sessionId, confidence, durationInSeconds);
    }
    router.push('/report');
  };
